...
# $Revision: 
# 07/14/2020: adding first set of script elements
# 07/28/2020: fixes for multiple items. Added backup and restore capabilities
######################################################################
...
echo "--Script CR_Utility.sh started"
...
# Here we assume Utility is to be set up as a log host
...
#RHEL-7:3:26, CIS 2.2.10. Remove FTP server. Solution: from CIS.
# Could not fallback
#yum remove vsftpd
#echo "---Start RHEL-7:3:26. Remove vsftpd ..."
#rpm -qa | grep -q vsftpd
#if [ $? -eq 0 ];then
#  yum remove vsftpd
#  echo -e "package: vsftpd removed"
#fi
#echo "---End of RHEL-7:3:26. Remove vsftpd"

#echo

#RHEL-7:3:27, CIS 2.2.11. Remove HTTP server. Solution: don't remove. Category: N/A
#yum remove httpd
...
#RHEL-7:6:57, CIS 2.2.2. Remove the X Window server. Solution: from spreadsheet.
# Could not fallback
#yum remove xorg-x11*
#echo "---Start RHEL-7:6:57. Remove the X Window System..."
#rpm -qa | grep -q xorg-x11
#if [ $? -eq 0 ];then
#  yum remove xorg-x11*
#  echo -e "package: xorg-x11* removed"
#fi
#echo "---End of RHEL-7:6:57. Remove the X Window System"
...
# Category: implemented. RHEL-7:6:63 - RHEL-7:6:65 could be accomplished by the same command
#echo "---Start RHEL-7:6:63,7:6:64,7:6:65. Add nodev,noexec,nosuid option for /tmp..."
#mount | grep -E '\s/tmp\s'
#if [ "$?" -eq 0 ]
#then
#  mount -o remount,noexec,nosuid,nodev /tmp
#fi
#echo "---End of RHEL-7:6:63,7:6:64,7:6:65. Add nodev,noexec,nosuid option for /tmp ..."

echo

#RHEL-7:6:67, CIS 1.7.1.4. Set SELinux State. Solution: from spreadsheet.
echo "---Start RHEL-7:6:67. Set SELinux State ..."
sed -i -E "s/.*SELINUX=.*/SELINUX=enforcing/g" /etc/selinux/config
echo "---End of RHEL-7:6:67. Set SELinux State"

echo

#RHEL-7:6:69, CIS 1.7.1.7. Uninstall SETroubleshoot. Solution: from spreadsheet.
# Could not fallback
#yum remove setroubleshoot
echo "---Start RHEL-7:6:69. Remove setroubleshoot..."
rpm -qa | grep -q setroubleshoot
if [ $? -eq 0 ];then
  yum remove setroubleshoot
  echo -e "package: setroubleshoot removed"
fi
echo "---End of RHEL-7:6:69. Remove setroubleshoot"

#RHEL-7:6:7, CIS N/A. Add nosuid option set for /home. Solution: from spreadsheet instruction.
# Commented out, Category: N/A. Conflicting with RHEL-7:6:7 which is compliant
